...
setwd("C:/Users/manuel.garcia/Desktop/TFM")
...
years_study <- datosMCSom$a?os_escolaridad
...
for(h in 1:S){ 
  ...
  mod <- mclapply(1:nummod, function(i){return(lmer(form_s[[i]], data = dat_s1_s, REML=T))}, mc.cores = cores)
  ...
  for(i in 1:nummod){
    ...
    sigmas[1,i] <- sigmas[1,i] + sigmae2est_s
    sigmas[2,i] <- sigmas[2,i] + sigmau2est_s
    ...
    for(j in 1:D){ #computing the maximum likelihood of the gof
      ...
      Qcaic <- Qcaic + sum(((Yd - mean_d - upred_s[j])^2))/sigmae2est
      
      #Components of rho_tau
      
      gamma_d <- sigmau2est_s/(sigmau2est_s + (sigmae2est_s/nd[j]))
      x_bar <- (1/nd[j])*(t(Xd)%*%rep(1, nd[j]))
      A <- A + t(Xd)%*%Xd - (nd[j]*gamma_d*(x_bar%*%t(x_bar)))
      B <- B + t(Xd)%*%Xd + (nd[j]*gamma_d)*(gamma_d-2)*(x_bar%*%t(x_bar))
      C <- C + gamma_d
      
    }
    
    Qfm_cond[i] <- Qcaic + n*log(sigmae2est) + 2*(ncol(Xs_s) + 2) #cAIC
    Qfm_alpha0[i] <- Q + n*log(sigmae2est) + D*log(sigmau2est) #alpha0
    Qfm_alpha1[i] <- Qfm_alpha0[i] + 2*(ncol(Xs_s) + 2) #alpha1
    
    A <- (1/sigmae2est_s)*A
    B <- (1/sigmae2est_s)*B
    rho_tau <- sum(diag((inv(A) %*% B))) + C
    
    Qfm_alpha2[i] <- Qfm_alpha0[i] + 2*(rho_tau + 2)#alpha2
  ...
  eblup <- mclapply(1:nummod, function(i){return(eblupBHF(formula = form_s_est[[i]], dom = municipio_s, meanxpop = means_mods[[i]],
                                                          popnsize = popsize, data = dat_s1_s)$eblup)}, mc.cores = cores)
  ...
  eb <- mclapply(1:nummod, function(i){return(ebBHF(formula = form_s_e_est[[i]], dom =  municipio_s, Xnonsample = data_mods_Non[[i]], constant = m,
                                                    indicator = pov_inc)$eb)}, mc.cores = cores)
...
tt_s <- Sys.time() - ini #8.853463 mins 1000 times
tt_s
...
for(i in 1:numgof){
  ...
  avg_gof[[i]] <- aux/S #Divide by the number of simulations to average
}
...
# The best model, in terms of averaged goodness of fit, is the model 5.
...
result_f0 <- result_f0[,c(5,1,4,3,2)]
...
res.my <- melt(result_y, measure.vars=c('Model 1','Model 2','Model 3', 'Model 4', 'Model 5'))
res.my <- res.my[,2:3]
colnames(res.my) <- c("Models", "MSE")
p1 <- ggplot(res.my) +
  geom_boxplot(aes(x=Models, y=MSE)) +
  theme_bw()

#These two variables make poor the estimation due to the low number of observatios, the variance of that variable increases the MSE
table(rur_urb_s, calidad_vivienda_s)

#Boxplot of MSE f_0

res.mf <- melt(result_f0, measure.vars=c('Model 1','Model 2','Model 3', 'Model 4', 'Model 5'))
res.mf <- res.mf[,2:3]
colnames(res.mf) <- c("Models", "MSE")
p2 <- ggplot(res.mf) +
  geom_boxplot(aes(x=Models, y=MSE)) +
  theme_bw()
...
#Scatterplots of maximum MSE (Y_bar and f_0) against each GOFs
...
df_qy <- matrix(0, ncol=5, nrow=1)
df_qf0 <- matrix(0, ncol=5, nrow=1)

for(i in 1:nummod){
  df_qy[1,i] <- mean(result_y[which(result_y[,i] > quantile(result_y[,i], 0.75)),i])
  df_qf0[1,i] <- mean(result_f0[which(result_f0[,i] > quantile(result_f0[,i], 0.75)),i])
}

#mean(result_y[which(result_y[,i] > quantile(result_y[,i], 0.75)),i])

colnames(df_my) <- c("index", "value")
colnames(df_mf0) <- c("index", "value")


gofsm <- matrix(0, nrow=numgof, ncol = nummod)
for(i in 1:numgof){
  gofsm[i,] <- avg_gof[[i]]
}
gofsm <- data.frame(gofsm)
gofsm <- gofsm[,c(5,1,4,3,2)]

colnames(gofsm) <- c("Model 1", "Model 2", "Model 3", "Model 4", "Model 5")

#cAIC
#Income

dataplota0_ym <- data.frame(t(rbind(gofsm[1,], df_my[,2])))
colnames(dataplota0_ym) <- c("cAIC", "MaxMSE")

dataplota0_yq <- data.frame(t(rbind(gofsm[1,], df_qy[1,])))
colnames(dataplota0_yq) <- c("cAIC", "ThirdqMSE")

p1 <- ggplot(dataplota0_ym, aes(MaxMSE, cAIC, label = rownames(dataplota0_ym))) + 
  geom_point() + geom_text(aes(label = rownames(dataplota0_ym), hjust = 0.5, vjust = -0.5)) +
  theme_bw() + ylim(c(-575, -450)) + xlim(c(-0.2, 0.8))

p2 <- ggplot(dataplota0_yq, aes(ThirdqMSE, cAIC, label = rownames(dataplota0_yq))) + 
  geom_point() + geom_text(aes(label = rownames(dataplota0_yq), hjust = 0.5, vjust = -0.5)) +
  theme_bw() + ylim(c(-575, -450)) + xlim(c(-0.2, 0.8))

grid.arrange(p1, p2, nrow = 1)

#Poverty
dataplota0_fm <- data.frame(t(rbind(gofsm[1,], df_mf0[,2])))
colnames(dataplota0_fm) <- c("cAIC", "MaxMSE")

dataplota0_fq <- data.frame(t(rbind(gofsm[1,], df_qf0[1,])))
colnames(dataplota0_fq) <- c("cAIC", "ThirdqMSE")

p1 <- ggplot(dataplota0_fm, aes(MaxMSE, cAIC, label = rownames(dataplota0_fm))) + 
  geom_point() + geom_text(aes(label = rownames(dataplota0_fm), hjust = 0.5, vjust = -0.5)) +
  theme_bw() + ylim(c(-575, -450)) + xlim(c(-0.01, 0.075))

p2 <- ggplot(dataplota0_fq, aes(ThirdqMSE, cAIC, label = rownames(dataplota0_fq))) + 
  geom_point() + geom_text(aes(label = rownames(dataplota0_fq), hjust = 0.5, vjust = -0.5)) +
  theme_bw() + ylim(c(-575, -450)) + xlim(c(-0.01, 0.075))
...
dataplota0_ym <- data.frame(t(rbind(gofsm[2,], df_my[,2])))
colnames(dataplota0_ym) <- c("GOF", "MaxMSE")

dataplota0_yq <- data.frame(t(rbind(gofsm[2,], df_qy[1,])))
colnames(dataplota0_yq) <- c("GOF", "ThirdqMSE")

p1 <- ggplot(dataplota0_ym, aes(MaxMSE, GOF, label = rownames(dataplota0_ym))) + 
  geom_point() + geom_text(aes(label = rownames(dataplota0_ym), hjust = 0.5, vjust = -0.5)) + 
  theme_bw() + ylim(c(-675, -550)) + xlim(c(-0.2, 0.8))

p2 <- ggplot(dataplota0_yq, aes(ThirdqMSE, GOF, label = rownames(dataplota0_yq))) + 
  geom_point() + geom_text(aes(label = rownames(dataplota0_yq), hjust = 0.5, vjust = -0.5)) +
  theme_bw() + ylim(c(-675, -550)) + xlim(c(-0.2, 0.8))
...
#Poverty
dataplota0_fm <- data.frame(t(rbind(gofsm[2,], df_mf0[,2])))
colnames(dataplota0_fm) <- c("GOF", "MaxMSE")

dataplota0_fq <- data.frame(t(rbind(gofsm[2,], df_qf0[1,])))
colnames(dataplota0_fq) <- c("GOF", "ThirdqMSE")

p1 <- ggplot(dataplota0_fm, aes(MaxMSE, GOF, label = rownames(dataplota0_fm))) + 
  geom_point() + geom_text(aes(label = rownames(dataplota0_fm), hjust = 0.5, vjust = -0.5)) +
  theme_bw() + ylim(c(-675, -550)) + xlim(c(-0.01, 0.075))

p2 <- ggplot(dataplota0_fq, aes(ThirdqMSE, GOF, label = rownames(dataplota0_fq))) + 
  geom_point() + geom_text(aes(label = rownames(dataplota0_fq), hjust = 0.5, vjust = -0.5)) +
  theme_bw() + ylim(c(-675, -550)) + xlim(c(-0.01, 0.075))
...
dataplota0_ym <- data.frame(t(rbind(gofsm[3,], df_my[,2])))
colnames(dataplota0_ym) <- c("GOF", "MaxMSE")

dataplota0_yq <- data.frame(t(rbind(gofsm[3,], df_qy[1,])))
colnames(dataplota0_yq) <- c("GOF", "ThirdqMSE")

p1 <- ggplot(dataplota0_ym, aes(MaxMSE, GOF, label = rownames(dataplota0_ym))) + 
  geom_point() + geom_text(aes(label = rownames(dataplota0_ym), hjust = 0.5, vjust = -0.5)) +
  theme_bw() + ylim(c(-625, -500)) + xlim(c(-0.2, 0.8))

p2 <- ggplot(dataplota0_yq, aes(ThirdqMSE, GOF, label = rownames(dataplota0_yq))) + 
  geom_point() + geom_text(aes(label = rownames(dataplota0_yq), hjust = 0.5, vjust = -0.5)) +
  theme_bw() + ylim(c(-625, -500)) + xlim(c(-0.2, 0.6))

grid.arrange(p1, p2, nrow = 1)

#Poverty
dataplota0_fm <- data.frame(t(rbind(gofsm[3,], df_mf0[,2])))
colnames(dataplota0_fm) <- c("GOF", "MaxMSE")

dataplota0_fq <- data.frame(t(rbind(gofsm[3,], df_qf0[1,])))
colnames(dataplota0_fq) <- c("GOF", "ThirdqMSE")

p1 <- ggplot(dataplota0_fm, aes(MaxMSE, GOF, label = rownames(dataplota0_fm))) + 
  geom_point() + geom_text(aes(label = rownames(dataplota0_fm), hjust = 0.5, vjust = -0.5)) +
  theme_bw() + ylim(c(-625, -500)) + xlim(c(-0.01, 0.075))
...
dataplota0_ym <- data.frame(t(rbind(gofsm[4,], df_my[,2])))
colnames(dataplota0_ym) <- c("GOF", "MaxMSE")

dataplota0_yq <- data.frame(t(rbind(gofsm[4,], df_qy[1,])))
colnames(dataplota0_yq) <- c("GOF", "ThirdqMSE")

p1 <- ggplot(dataplota0_ym, aes(MaxMSE, GOF, label = rownames(dataplota0_ym))) + 
  geom_point() + geom_text(aes(label = rownames(dataplota0_ym), hjust = 0.5, vjust = -0.5)) +
  theme_bw() + ylim(c(-600, -400)) + xlim(c(-0.2, 0.8))

p2 <- ggplot(dataplota0_yq, aes(ThirdqMSE, GOF, label = rownames(dataplota0_yq))) + 
  geom_point() + geom_text(aes(label = rownames(dataplota0_yq), hjust = 0.5, vjust = -0.5)) +
  theme_bw() + ylim(c(-600, -400)) + xlim(c(-0.2, 0.6))

grid.arrange(p1, p2, nrow = 1)

#Poverty
dataplota0_fm <- data.frame(t(rbind(gofsm[4,], df_mf0[,2])))
colnames(dataplota0_fm) <- c("GOF", "MaxMSE")

dataplota0_fq <- data.frame(t(rbind(gofsm[4,], df_qf0[1,])))
colnames(dataplota0_fq) <- c("GOF", "ThirdqMSE")

p1 <- ggplot(dataplota0_fm, aes(MaxMSE, GOF, label = rownames(dataplota0_fm))) + 
  geom_point() + geom_text(aes(label = rownames(dataplota0_fm), hjust = 0.5, vjust = -0.5)) +
  theme_bw() + ylim(c(-600, -400)) + xlim(c(-0.01, 0.075))
...
#save(list = ls(), file = "simulation.RData")

#Conclusion MSE, removing low significant variables affects, but GOF are not important when selecting models.